...
export default function HistoryPage() {
  ...
  const stats = {
    ...
    completedPercent: Math.round(
      (deliveries.filter((d) => d.status === "completed" || d.status === "refunded").length / deliveries.length) *
        100,
    ),
  ...
  return (
    <AppShell>
      ...
      {/* INSIGHT */}
      <div className="mx-4 mb-3 p-3.5 bg-gradient-to-bl from-cyan/[0.06] to-navy/[0.04] border border-cyan/20 rounded-[16px] flex gap-3">
        <div className="w-9 h-9 rounded-[10px] bg-cyan flex items-center justify-center flex-shrink-0">
          <CheckCircle2 size={18} className="text-white" strokeWidth={2.5} />
        </div>
        <div className="flex-1 text-[12px] leading-snug">
          <div className="text-[12px] font-bold text-navy mb-0.5">📊 תובנה: רמי לוי הכי בעייתי</div>
          <div className="text-text-sub">
            3 מתוך 5 משלוחי רמי לוי האחרונים הגיעו עם פערים. שופרסל ב-0%.
          </div>
        </div>
      </div>
      ...
      {/* GROUPS */}
      {filtered.length === 0 && (
        <div className="text-center text-text-sub py-12">לא נמצאו משלוחים</div>
      )}
...
function DeliveryRow({ delivery }: { delivery: Delivery }) {
  ...
  const accent =
    refunded ? "after:bg-green" : issues ? "after:bg-coral" : "after:bg-transparent";
  ...
  return (
    ...
    >
      <div
        className={cn(
          "w-11 h-11 rounded-[12px] bg-gradient-to-br flex items-center justify-center text-white font-display font-bold text-[13px] flex-shrink-0",
          logo.gradient,
        )}
      >
        {logo.initials}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <div className="text-[13px] font-semibold text-navy flex-1 truncate">
            {delivery.storeName} — #{delivery.orderNumber.slice(-6)}
          </div>
...
function StatusBadge({ delivery }: { delivery: Delivery }) {
  if (delivery.status === "refunded") {
    return (
      <span className="bg-cyan/15 text-[#0369a1] px-2.5 py-0.5 rounded-full text-[10px] font-bold">
        ✓ החזר{delivery.refundAmount ? ` $${delivery.refundAmount}` : ""} התקבל
      </span>
    );
  }
  if (hasIssues(delivery)) {
    ...
    return (
      <span className="bg-coral/15 text-[#be123c] px-2.5 py-0.5 rounded-full text-[10px] font-bold">
        {issueCount} פערים
      </span>
    );
  }
  if (delivery.status === "completed") {
    return (
      <span className="bg-green/15 text-[#15803d] px-2.5 py-0.5 rounded-full text-[10px] font-bold">
        תקין
      </span>
    );
  }
  return (
    <span className="bg-yellow/20 text-[#b45309] px-2.5 py-0.5 rounded-full text-[10px] font-bold">
      בהמתנה למענה
    </span>
  );
}
