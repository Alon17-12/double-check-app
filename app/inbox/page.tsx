...
import { mockUserInbox, mockIntakes, supportedVendors } from "@/lib/mocks/inbox-data";

export default function InboxSetupPage() {
  const [copied, setCopied] = useState(false);
  const inbox = mockUserInbox;

  const copyAddress = async () => {
    await navigator.clipboard.writeText(inbox.inboxFull);
    setCopied(true);
  ...
  return (
    <AppShell>
      ...
      <div className="relative bg-gradient-to-br from-navy-dark via-navy to-cyan text-white px-5 pt-6 pb-16 overflow-hidden">
        <div className="absolute -top-12 -left-12 text-[200px] opacity-[0.08] blur-sm pointer-events-none select-none leading-none">
          📨
        </div>
      ...
      <div className="-mt-10 mx-4 bg-surface rounded-[22px] p-5 shadow-[0_4px_14px_rgba(15,23,42,0.08)] relative z-10">
        ...
        <div className="bg-gradient-to-bl from-cyan/[0.08] to-navy/[0.04] border-2 border-dashed border-cyan/30 rounded-[16px] p-4 text-center mb-3.5">
          ...
          <div className="text-[11px] text-text-sub">
            הכתובת האישית שלך — שלח כל אישור הזמנה לכאן
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button className="flex-1 bg-navy text-white py-3 rounded-[12px] font-semibold text-[13px] flex items-center justify-center gap-1.5">
      ...
      <section className="px-4 mt-6">
        ...
        <Step
          n={1}
          title="העברה ידנית"
          body="קיבלת אישור הזמנה? לחץ Forward → הדבק את הכתובת שלך → שלח. תוך 5 שניות תקבל הודעה שההזמנה נקלטה."
        />
        <Step
          n={2}
          title="פילטר אוטומטי ב-Gmail"
          body="Settings → Filters → Create filter → From: no-reply@amazon.com → Forward to: הכתובת שלך. עכשיו כל הזמנה מאמזון נכנסת אוטומטית."
        />
        <Step
          n={3}
          title="שיתוף בוואטסאפ לעצמך"
          body="שמור את הכתובת ב-Saved Messages — תוכל להעתיק במהירות מכל מקום."
        />
      </section>
      ...
      <section className="px-4 mt-5 space-y-2.5">
        ...
        >
          📥 צפה ב-{inbox.totalEmails} מיילים שנקלטו
          <ChevronLeft size={16} />
        </Link>
      </section>
      ...
      <section className="mx-4 mt-5 bg-surface rounded-[16px] border border-border p-4">
        ...
        <div className="text-[11px] text-text-sub mb-3 leading-relaxed">
          פרסור ייעודי לדיוק 99%. כל ספק אחר עובד דרך Gemini Generic.
        </div>
      ...
      <div className="mx-4 my-5 p-3.5 bg-yellow/10 border border-yellow/30 rounded-[12px] flex gap-2.5 text-[12px] leading-snug">
        ...
        <div>
          <strong className="text-[#b45309]">פרטיות:</strong>{" "}
          תוכן המיילים נמחק אחרי 30 יום. רק נתוני ההזמנה נשמרים. אפשר לאפס את הכתובת בכל שלב אם הגיע spam.
        </div>
      </div>
    </AppShell>
  );
...
function Step({ n, title, body }: { n: number; title: string; body: string }) {
  return (
    <div className="bg-surface border border-border rounded-[12px] p-3.5 mb-2 flex gap-3">
      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-cyan to-cyan-dark text-white flex items-center justify-center font-display text-[13px] font-bold flex-shrink-0">
        {n}
      </div>
