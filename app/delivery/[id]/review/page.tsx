...
export default function ReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const setItem = useDeliveryStore((s) => s.setItem);
  const removeItem = useDeliveryStore((s) => s.removeItem);
  const [editingId, setEditingId] = useState<string | null>(null);

  if (!delivery) {
    return <div className="p-8 text-center">משלוח לא נמצא</div>;
  }

  const totalQty = delivery.items.reduce((sum, i) => sum + i.orderedQty, 0);
    ...
    <div className="mx-auto max-w-[480px] min-h-screen bg-bg pb-[100px] md:my-6 md:rounded-[28px] md:overflow-hidden md:shadow-md md:min-h-[calc(100vh-48px)]">
      ...
      <div className="mx-4 mt-3 p-3.5 bg-green/10 border border-green/20 rounded-[12px] flex items-center gap-3 text-xs leading-snug">
        ...
        <div>
          ...
          <br />
          <span className="text-text-sub">חולצו {delivery.items.length} פריטים מקבלת {delivery.storeName}</span>
        </div>
      </div>

      {/* SUMMARY BAR */}
      <div className="mx-4 mt-3 mb-3 bg-surface border border-border rounded-[12px] flex overflow-hidden text-[11px]">
        <SummaryCell value={String(delivery.items.length)} label="פריטים" />
        <SummaryCell value={totalQty.toFixed(2)} label="סה״כ כמות" />
        <SummaryCell value={formatILS(delivery.ilsTotal).replace(".77", "")} label="סך עם מע״מ" />
      </div>

      {/* ITEMS */}
      <div className="px-4">
        <div className="text-[12px] font-semibold text-text-sub uppercase tracking-wider mb-2 pr-1">
          פריטי הקבלה
        </div>
          ...
          {delivery.items.map((item) => {
            ...
            return isEditing ? (
              <ItemEditRow
                ...
                item={item}
                onSave={(patch) => {
                  setItem(delivery.id, item.id, patch);
                  setEditingId(null);
                }}
                onCancel={() => setEditingId(null)}
                onDelete={() => {
                  removeItem(delivery.id, item.id);
                  setEditingId(null);
                }}
              />
            ) : (
        ...
        <div className="mt-3 bg-surface border border-border rounded-[12px] p-3.5">
          ...
          <TotalRow label="מע״מ 18%" value={formatILS(delivery.vatAmount)} />
...
function metaLine(item: { unit: string; orderedQty: number; unitPrice: number; totalPrice: number }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-[12px] p-3 flex items-center gap-2.5 hover:border-cyan/40 hover:shadow-sm transition">
      ...
      <button
        onClick={onEdit}
        aria-label="ערוך"
        className="w-[30px] h-[30px] rounded-lg bg-bg flex items-center justify-center text-text-sub hover:bg-[rgba(30,58,138,0.08)] hover:text-navy transition"
      >
        <Pencil size={14} />
      </button>
    </div>
...
function ItemEditRow({
  ...
  item: { id: string; position: number; name: string; orderedQty: number; unit: string; unitPrice: number; totalPrice: number };
    ...
    <div className="bg-surface border-cyan border rounded-[12px] shadow-[0_0_0_3px_rgba(56,196,242,0.15)] p-3 flex items-start gap-2.5">
      ...
      <div className="flex-1 flex flex-col gap-1.5">
        <input
          ...
          className="w-full px-2.5 py-1.5 border border-border rounded-md bg-bg text-[13px]"
        />
        <div className="flex gap-1.5">
          <input
            ...
            step="0.01"
            placeholder="כמות"
            className="flex-1 px-2.5 py-1.5 border border-border rounded-md bg-bg text-[13px]"
          />
          <input
            value={price}
        ...
        <div className="flex gap-1.5 mt-1">
          ...
          <button
            onClick={onCancel}
            className="px-3 bg-bg text-text-sub py-2 rounded-md text-[12px] font-semibold"
          >
            ביטול
          </button>
        </div>
      ...
      <button
        onClick={onDelete}
        aria-label="מחק"
        className="w-[30px] h-[30px] rounded-lg bg-bg flex items-center justify-center text-text-sub hover:bg-coral/10 hover:text-coral transition mt-1"
      >
        <Trash2 size={14} />
      </button>
    </div>
