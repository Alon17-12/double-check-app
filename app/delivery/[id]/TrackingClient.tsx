...
import { setItemStatus } from "@/lib/data/actions";
...
export function TrackingClient({ delivery }: { delivery: Delivery }) {
  const router = useRouter();
  const [items, setItems] = useState(delivery.items);
  const [filter, setFilter] = useState<Filter>("all");
  const [, startTransition] = useTransition();

  const progress = useMemo(() => computeProgress({ ...delivery, items }), [delivery, items]);
  ...
  const handleStatusChange = (itemId: string, status: ItemStatus) => {
    ...
    startTransition(async () => {
      ...
      if (!item) return;
      const receivedQty = status === "ok" ? item.orderedQty : status === "missing" ? 0 : (item.receivedQty ?? undefined);
      try {
        await setItemStatus(itemId, status, receivedQty);
      } catch (e) {
        console.error(e);
      }
    });
  ...
  return (
    <div className="mx-auto max-w-[480px] min-h-screen bg-bg pb-[100px] md:my-6 md:rounded-[28px] md:overflow-hidden md:shadow-md md:min-h-[calc(100vh-48px)]">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-[14px] font-bold text-navy truncate">
              {delivery.storeName} {delivery.orderNumber ? `— הזמנה #${delivery.orderNumber}` : ""}
            </h1>
      ...
      <div className="px-3 flex flex-col gap-2">
        ...
        {filtered.map((item) => (
          <ItemRow key={item.id} item={item} onChange={(status) => handleStatusChange(item.id, status)} />
        ))}
      </div>
...
function Stat({ dot, label, count }: { dot: "green" | "orange" | "coral" | "gray"; label: string; count: number }) {
  const colors = { green: "bg-green", orange: "bg-orange", coral: "bg-coral", gray: "bg-text-muted" };
  return (
    ...
    <div className="mx-4 mt-3 bg-gradient-to-bl from-orange/10 to-orange/[0.03] border border-orange/30 rounded-[14px] p-3.5">
      ...
      <div className="flex items-baseline gap-2 text-[14px]">
        <span className="font-display text-navy font-bold">
          {symbol}
          {delivery.originalTotal.toFixed(2)}
        </span>
        <span className="text-text-muted">≈</span>
        <span className="font-display text-text-sub font-semibold">{formatILS(delivery.ilsTotal)}</span>
      </div>
...
function ItemRow({ item, onChange }: { item: DeliveryItem; onChange: (s: ItemStatus) => void }) {
  ...
  return (
    <div className={cn("border rounded-[12px] p-3 flex items-center gap-3 transition-all relative overflow-hidden", tone.bg, tone.border)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11px] text-text-sub flex gap-1.5 items-center">
          {item.status === "missing" ? (
            ...
            <>
              הוזמן {item.orderedQty}, הגיע <strong className="text-orange">{item.receivedQty ?? "—"}</strong> • חוסר{" "}
            ...
            <>
              <strong className="font-display text-navy">{item.orderedQty} {unitLabel}</strong>
      ...
      {item.status !== "pending" && item.status !== "ok" && <QuickStatus current={item.status} onChange={onChange} />}
...
function Checkbox({ status, onClick }: { status: ItemStatus; onClick: () => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        "w-[30px] h-[30px] border-2 rounded-[9px] flex items-center justify-center flex-shrink-0 transition-colors text-white",
        styles[status],
      )}
    >
