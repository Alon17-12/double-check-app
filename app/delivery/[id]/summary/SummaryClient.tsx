...
export function SummaryClient({
  ...
  const refundPct = delivery.ilsTotal > 0 ? ((refund / delivery.ilsTotal) * 100).toFixed(1) : "0";
      ...
      {issueCount > 0 && (
        <>
          <div className="mx-4 mt-4 p-5 bg-gradient-to-bl from-coral/10 to-coral/[0.02] border-[1.5px] border-coral/25 rounded-[22px] overflow-hidden">
            <div className="text-[11px] text-coral font-bold uppercase tracking-wider mb-1">סך החזר מבוקש</div>
          ...
          <div className="px-4 mt-4">
            ...
            {missing.length > 0 && (
              <DiscrepancyGroup
                ...
                items={missing.map((i) => ({ name: i.name, qty: `${i.orderedQty} יח׳`, price: i.totalPrice }))}
              />
            )}
            {partial.length > 0 && (
              <DiscrepancyGroup
            ...
            {damaged.length > 0 && (
              <DiscrepancyGroup
                ...
                items={damaged.map((i) => ({ name: i.name + (i.photoUrl ? " 📸" : ""), qty: i.issueNote ?? "פגום", price: i.totalPrice }))}
              />
            )}
          </div>
        </>
...
function DiscrepancyGroup({ icon, tone, title, subtitle, amount, items }: { icon: string; tone: "coral" | "orange" | "yellow"; title: string; subtitle: string; amount: number; items: Array<{ name: string; qty: string; price: number }> }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-[16px] mb-3 overflow-hidden">
      <div className="p-3.5 flex items-center gap-2.5 border-b border-border">
        <div className={`w-8 h-8 rounded-[9px] flex items-center justify-center text-base flex-shrink-0 ${colorClass}`}>{icon}</div>
        <div className="flex-1">
          <div className="text-[13px] font-bold text-navy">{title}</div>
          <div className="text-[11px] text-text-sub">{subtitle}</div>
        </div>
        <div className={`font-display text-[14px] font-bold ${amountClass}`}>{formatILS(amount)}</div>
      </div>
      <div className="py-1">
        {items.map((it, i) => (
