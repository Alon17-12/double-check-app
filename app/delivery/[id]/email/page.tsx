...
export default function EmailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const email = useMemo(
    () => (delivery ? generateEmail(delivery, tone) : null),
    [delivery, tone],
  );

  if (!delivery || !email) return <div className="p-8 text-center">משלוח לא נמצא</div>;
  ...
  return (
    <div className="mx-auto max-w-[480px] min-h-screen bg-bg pb-[100px] md:my-6 md:rounded-[28px] md:overflow-hidden md:shadow-md md:min-h-[calc(100vh-48px)]">
      ...
      {/* TONE TOGGLE */}
      <div className="p-4">
        <div className="text-[11px] text-text-sub font-semibold uppercase tracking-wider mb-2 pr-1">
          סגנון פנייה
        </div>
      ...
      <div className="mx-4 mb-3 bg-surface border border-border rounded-[12px] p-3.5">
        <div className="flex items-center gap-2.5 text-xs">
          ...
          {supportEmail && (
            <span className="bg-bg text-cyan rounded-md px-2.5 py-1 text-[11px] font-semibold">
              ⚡ זוהה אוטומטית
            </span>
          )}
        </div>
      ...
      <div className="mx-4 bg-surface rounded-[16px] border border-border overflow-hidden shadow-sm">
        <div className="px-4 py-3.5 border-b border-border bg-gradient-to-bl from-navy/[0.03] to-cyan/[0.03]">
          ...
          <div className="text-[11px] text-text-sub">
            משלוח מ-{formatDate(delivery.deliveryDate ?? delivery.orderDate)} • {issueGroups.missing.length + issueGroups.partial.length + issueGroups.damaged.length} פערים • החזר {formatILS(email.refundAmount)}
          </div>
        </div>
        <div className="p-4 text-[13px] leading-[1.85]">
          ...
          {issueGroups.partial.length > 0 && (
            <Section emoji="⚠️" title={`פריטים שהגיעו בכמות חלקית (${issueGroups.partial.length}):`} color="text-[#b45309]">
              {issueGroups.partial.map((i) => (
                <li key={i.id}>
                  <strong>{i.name}</strong> — הוזמנו {i.orderedQty} יח׳, הגיעו {i.receivedQty ?? 0} — חוסר {formatILS(i.unitPrice * (i.orderedQty - (i.receivedQty ?? 0)))}
                </li>
              ))}
            </Section>
          )}
          ...
          <p className="mb-2">
            אשמח לקבל החזר כספי / זיכוי / משלוח חוזר של הפריטים החסרים. מצורפת הקבלה המלאה לעיון.
          </p>
          <p>
            תודה,<br />
            <strong>אלון גבאי</strong>
            <br />
            050-XXX-XXXX
          </p>
        </div>
      ...
      >
        <Button variant="secondary" size="lg" aria-label="ערוך מייל">
          <Pencil size={16} />
        </Button>
...
function ToneBtn({
  ...
  return (
    <button
      onClick={() => onSet(value)}
      className={cn(
        "flex-1 py-2 px-2 rounded-lg flex flex-col items-center gap-0.5 font-semibold transition-all",
        isActive
          ? "bg-navy text-white shadow-[0_4px_12px_rgba(30,58,138,0.2)]"
...
function ShareBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="bg-surface border border-border rounded-[12px] p-3.5 cursor-pointer flex flex-col items-center gap-1 hover:border-cyan hover:-translate-y-0.5 hover:shadow-sm transition-all"
    >
      <div className={cn("w-9 h-9 rounded-[10px] flex items-center justify-center mb-0.5", bg)}>
        {icon}
      </div>
